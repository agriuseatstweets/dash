...
def filter(event)
  ...
  if event.get("retweeted_status")
    ...
    _trim_subtweet(event, "retweeted_status", ["user", "id_str"])
  end
  ...
  if event.get("quoted_status")
    ...
    _trim_subtweet(event, "quoted_status", ["user", "id_str", "entities", "text"])
  end
...
test "user mentions flattened" do
  in_event { { "entities" => { "user_mentions" => [{"screen_name" => "foo"}, {"screen_name" => "bar"}] } } }
...
test "hashtags flattend" do
  in_event { { "entities" => { "hashtags" => [{"text" => "foo"}, {"text" => "bar"}] } } }
